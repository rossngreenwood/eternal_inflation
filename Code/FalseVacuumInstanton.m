...
properties ( SetAccess = immutable )
    ...
    kappa   = 8*pi/3    % $\kappa = 8\pi(G/3) = 8\pi/(3 M_Pl^2)$
...
methods (Access = protected)
    ...
    function [r,y,convergence_type] = integrate_profile(self,r0,y0,dr0,epsabs,drmin,rmax)
        ...
        while true
            ...
            [r1,y1] = ode45(@(r,y) dY(y,r).',[r0,r0+0.5*dr,r0+dr],y0.');
    ...
    function [R_int,Y_int] = make_interior_points(self,R,delta_phi0,max_interior_pts)
...
methods
    ...
    function dy = equation_of_motion(self,y,~)
        ...
        %     -\frac{8\pi}{3M_{Pl}^2}\left[\left(\frac{d\phi}{dr}\right)^2
        %     + V(\phi)\right]
%         d2rho = -self.kappa*rho.*(dphi.*dphi + self.V(phi));
%         d2rho = self.kappa*rho.*(0.5*dphi.*dphi + self.V(phi)) + self.kappa*rho.^2./drho.*dphi;
    ...
    function [R,Y,Rerr,n_interior_pts] = find_profile(self,xguess,xtol,phitol,...
        ...
        while true
            ...
            % Integrate profile
            [rf,~,ctype] = self.integrate_profile(r0,y0,dr0,epsabs,drmin,rmax);
        ...
        [R,Y,Rerr] = self.integrate_and_save_profile(linspace(r0,rf*1.3,npoints),y0,dr0,drmin);
        ...
        [R_int,Y_int] = make_interior_points(self,R,delta_phi0,max_interior_pts);
        
        n_interior_pts = length(R_int);
...
methods
    ...
    function S = find_action(self,R,Y,Phi_int)
        ...
        % Add gravitational (Einstein-Hilbert) term to the action
        % Nope, we're not doing quantum gravity here
        if false
            Rho_fv   = [];
            dRho_fv  = zeros(size(Rho));
            d2Rho_fv = zeros(size(Rho));
            lagr_EH = 3/self.kappa*(...
                (Rho.^2.*d2Rho + Rho.*(dRho).^2 + Rho) - ...
                (Rho_fv.^2.*d2Rho_fv + Rho_fv.*(dRho_fv).^2 + Rho_fv) );
            S_EH = traps(R,lagr_EH.*area);
            S = S + S_EH;
        end
